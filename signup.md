...
<body>
    <div class="container">
        <form onsubmit="signUpRequest(); return false;">
...
<script>
    ...
    //  var url = "https://ccplace.stu.nighthawkcodingsociety.com";
        var url = "http://localhost:8085";
            ...
            fetch('https://ccplace.stu.nighthawkcodingsociety.com/api/person/post', {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                    name: fetchName,
                    email: fetchEmail,
                    password: `${fetchPassword}@123`,
                    dob: fetchDob,
                }),
            })
                .then(response => {
                    if (!response.ok) {
                        const errorMsg = 'Sign Up error: ' + response.status;
